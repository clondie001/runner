...
export const Player: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const onControlLeft = () => moveLeft();
    const onControlRight = () => moveRight();
    const onControlJump = () => triggerJump();
    const onControlSpecial = () => activateImmortality();

    window.addEventListener('keydown', handleKeyDown);
    window.addEventListener('control-left', onControlLeft);
    window.addEventListener('control-right', onControlRight);
    window.addEventListener('control-jump', onControlJump);
    window.addEventListener('control-special', onControlSpecial);

    return () => {
        window.removeEventListener('keydown', handleKeyDown);
        window.removeEventListener('control-left', onControlLeft);
        window.removeEventListener('control-right', onControlRight);
        window.removeEventListener('control-jump', onControlJump);
        window.removeEventListener('control-special', onControlSpecial);
    };
  ...
  return (
    <group ref={groupRef} position={[0, 0, 0]}>
      <group ref={bodyRef} position={[0, 1.1, 0]}> 
        ...
        <mesh position={[0.08, 0.1, -0.28]} geometry={GLOW_STRIP_GEO} material={glowMaterial} />
        <mesh castShadow position={[0, 0.6, 0]} geometry={HEAD_GEO} material={armorMaterial} />
